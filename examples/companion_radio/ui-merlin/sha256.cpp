...
void sha256(const void *data, size_t len, uint8_t out[32]) {
  ...
  size_t full = len / 64;
  for (size_t i = 0; i < full; ++i) block(h, p + i * 64);

  // Tail: the remaining bytes, the 0x80 marker, zero padding, and the bit
  // count as a big-endian 64-bit value. Two blocks are enough for any tail.
  uint8_t tail[128] = {0};
